...
#include "customWidget_forSPRC.cpp"
...
int three_frames(){
	...
	auto frame2 = AddFrame(myWnd, "frame 2", Vector2d(30,10), Vector2d(200,500));
	auto frame3 = AddFrame(myWnd, "frame 3", Vector2d(200,10), Vector2d(200,600));
	auto frame4 = AddFrame(myWnd, "frame 4", Vector2d(400,10), Vector2d(200,600));
	...
	// MY_ASSERT(AddButton(frame2, &test, "test2", Vector2d(30,40), Vector2d(100, 40)));
...
int test2(){
	...
	frame->setAlignType(glUI::uiWidgetAlignTypes::VertialListl);
	myWnd->setAlignType(glUI::uiWidgetAlignTypes::Horizontel_noSpace);
	frame->setEnableUserResize(false);

	MY_ASSERT(AddLabel(frame,"lavel1", Vector2d(10,30), Vector2d(100,50)));
	MY_ASSERT(AddTextBox(frame, &str1, Vector2d(10,60), Vector2d(600,200)));
	auto listBox = AddListBox(frame, "List__ 1\rList__2\rList__3\rLisst__4\r", &vv, Vector2d(10,60), Vector2d(600,200));
	listBox->setParamString("title", "これはテスト");


	MY_ASSERT(AddCheckbox(frame, &test, "test1", Vector2d(30,20), Vector2d(80, 120)));
	MY_ASSERT(AddButton(frame, &test, "test2", Vector2d(30,40), Vector2d(100, 40)));

	for(int i=0; i<3; i++){
		sprintf (buffer, "test = %d", i);
		AddButton(frame, &test, buffer, Vector2d(30,60), Vector2d(100, 40));
		AddButton(frame, &test, buffer, Vector2d(30,60), Vector2d(100, 40));
		AddButton(frame, &test, buffer, Vector2d(30,60), Vector2d(100, 40));
		AddButton(frame, &test, buffer, Vector2d(30,60), Vector2d(100, 40));
		AddButton(frame, &test, buffer, Vector2d(30,60), Vector2d(100, 40));
